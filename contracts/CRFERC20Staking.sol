...
contract CRFERC20Staking is Ownable, Pausable, ReentrancyGuard {

    using Address for address;
    using SafeMath for uint256;

    uint256 private constant SHARE_UNIT = 1e12;
    uint256 public immutable REWARD_UNIT;
    uint256 public immutable ERC20_DECIMALS;
    uint256 public immutable ERC20_STAKE_UNIT;

    IERC20 public immutable AssetTokenContract;
    IERC20 public immutable RewardTokenContract;
    address public immutable assetTokenAddress;
    ...
    constructor(
        ...
        uint256 erc20Decimals,
        uint256 rewardUnit
    )
    {
        require(assetTokenAddress_ != address(0) && rewardTokenAddress_ != address(0), "Zero address is not allowed");
        _totalStakeBlocks = totalStakeBlocks;
        assetTokenAddress = assetTokenAddress_;
        rewardTokenAddress = rewardTokenAddress_;
        AssetTokenContract = IERC20(assetTokenAddress_);
        RewardTokenContract = IERC20(rewardTokenAddress_);
        ERC20_DECIMALS = erc20Decimals;
        ERC20_STAKE_UNIT = 10 ** erc20Decimals / 10;
        REWARD_UNIT = rewardUnit * 1e15;
        _pause();
    }
    ...
    function start() external nonReentrant whenPaused onlyOwner {
        ...
        _totalStakedEth = ERC20_STAKE_UNIT;
    ...
    function totalUnclaimedReward(address staker) external view returns(uint256) {
        if(_stakeAmount[staker] == 0) {
            ...
            if(blockTotalEth[_secondLastStakedBlock] == 0) {
                blockEthShare = SHARE_UNIT.mul(ERC20_STAKE_UNIT).div(_totalStakedEth);
            } else {
                blockEthShare = SHARE_UNIT.mul(ERC20_STAKE_UNIT).div(blockTotalEth[_secondLastStakedBlock]);
            }
            ...
            return _untakedRewards[staker] + _stakeAmount[staker].mul(accShare).mul(REWARD_UNIT).div(SHARE_UNIT).div(ERC20_STAKE_UNIT);
        }
    }
    ...
    function _calcRewards(address staker, uint256 accShare) internal view returns(uint256) {
        ...
        uint256 reward = amount.mul(share).mul(REWARD_UNIT).div(SHARE_UNIT).div(ERC20_STAKE_UNIT);
        return reward;
    }

    function stake(uint256 stakedWei) external nonReentrant whenNotPaused {
        ...
        AssetTokenContract.transferFrom(staker, address(this), stakedWei);
    ...
    function unstake(uint256 unstakedWei) external nonReentrant whenNotPaused {
        ...
        if(blocknum == _lastStakedBlock) {
            ...
            uint256 blockEthShare = SHARE_UNIT.mul(ERC20_STAKE_UNIT).div(_totalStakedEth);
            _blockEthShare[_lastStakedBlock] = blockEthShare;
        ...
        AssetTokenContract.transfer(staker, unstakedWei);
    ...
    function claim() external nonReentrant whenNotPaused {
        ...
        require(RewardTokenContract.transfer(staker, reward), "Transfer token failed.");

        emit Claim(staker, block.number, reward);
    }

    function returnTokens(address reciever, uint256 amount) external onlyOwner {
        RewardTokenContract.transfer(reciever, amount);
    }
}