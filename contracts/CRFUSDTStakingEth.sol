...
contract CRFUSDTStakingEth is Ownable, Pausable, ReentrancyGuard {

    using Address for address;
    using SafeMath for uint256;

    uint256 private constant REWARD_UNIT = 1500 * 1e15;
    uint256 private constant SHARE_UNIT = 1e12;
    uint256 public constant USDT_DECIMALS = 6;
    uint256 public constant USDT_STAKE_UNIT = 10 ** USDT_DECIMALS / 10;

    IERC20 private immutable _TokenContract;
    IUSDT private _USDTContract;
    ...
    function setUsdt(address usdtAddress) external onlyOwner {
        require(usdtAddress != address(0), "Zero address is not allowed");
        _usdtAddress = usdtAddress;
        _USDTContract = IUSDT(usdtAddress);
    }
    ...
    function start() external nonReentrant whenPaused onlyOwner {
        ...
        _totalStakedEth = USDT_STAKE_UNIT;
    ...
    function totalUnclaimedReward(address staker) external view returns(uint256) {
        if(_stakeAmount[staker] == 0) {
            ...
            if(blockTotalEth[_secondLastStakedBlock] == 0) {
                blockEthShare = SHARE_UNIT.mul(USDT_STAKE_UNIT).div(_totalStakedEth);
            } else {
                blockEthShare = SHARE_UNIT.mul(USDT_STAKE_UNIT).div(blockTotalEth[_secondLastStakedBlock]);
            }
            ...
            return _untakedRewards[staker] + _stakeAmount[staker].mul(accShare).mul(REWARD_UNIT).div(SHARE_UNIT).div(USDT_STAKE_UNIT);
        }
    }
    ...
    function _calcRewards(address staker, uint256 accShare) internal view returns(uint256) {
        ...
        uint256 reward = amount.mul(share).mul(REWARD_UNIT).div(SHARE_UNIT).div(USDT_STAKE_UNIT);
        return reward;
    }
    ...
    function stake(uint256 amount) external nonReentrant whenNotPaused {
        ...
        uint256 stakedWei;
        uint256 usdtBasicPointsRate = _USDTContract.basisPointsRate();
        uint256 usdtMaximumFee = _USDTContract.maximumFee();
        uint256 fee = amount.mul(usdtBasicPointsRate).div(10000);
        if(fee > usdtMaximumFee) {
            fee = usdtMaximumFee;
        }
        stakedWei = amount - fee;
        ...
        _USDTContract.transferFrom(staker, address(this), stakedWei);
    ...
    function unstake(uint256 amount) external nonReentrant whenNotPaused {
        ...
        uint256 unstakedWei = amount - fee;
        ...
        if(blocknum == _lastStakedBlock) {
            ...
            uint256 blockEthShare = SHARE_UNIT.mul(USDT_STAKE_UNIT).div(_totalStakedEth);
            _blockEthShare[_lastStakedBlock] = blockEthShare;
        ...
        _USDTContract.transfer(staker, unstakedWei);
